...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm w-full max-w-full">
      {/* Top Ticker Bar */}
      ...
      {/* Main Navbar Branding & Controls Row */}
        ...
        <div className="flex items-center justify-between h-16 gap-3">
          ...
          >
            ...
            <div>
              ...
              <span className="block text-[9px] text-slate-500 font-medium -mt-1 hidden sm:block">Direct Harvest & Agronomy</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto">
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-xl bg-slate-100 text-slate-700 shrink-0 hover:bg-slate-200 transition-colors"
      ...
      {/* Modules Sub-Navigation Bar - Clean, Non-overlapping, Horizontally Scrollable */}
      <div className="border-t border-slate-200/80 bg-slate-50/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <nav className="flex items-center gap-1.5 sm:gap-2 py-2 overflow-x-auto no-scrollbar scrollbar-none whitespace-nowrap">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shrink-0 ${
                    isActive 
                      ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-600/20 font-bold' 
                      : 'bg-white text-slate-700 hover:text-emerald-700 hover:bg-emerald-50/80 border border-slate-200/80'
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-white' : 'text-emerald-600'}`} />
                  <span>{item.label}</span>
                  {item.badge && (
                    <span className={`ml-0.5 text-[9px] font-bold px-1.5 py-0.2 rounded-full ${
                      isActive 
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-b border-slate-200 px-4 pt-3 pb-5 space-y-3 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && (
                    <span className="ml-auto text-[9px] font-bold bg-amber-500 text-white px-1.5 py-0.2 rounded-full shrink-0">
                      {item.badge}
                    </span>
                  )}
                </button>
